...
interface SearchBehavior<T, S> //or SearchStrategy. Interface class (has no member variables).
{// T is the object(like class Student, AllStudents), S is the value.
    boolean search(T obj, S v); //Every class using this method has to override to use this method.
}
class StudentSearch implements SearchBehavior<Student, String>
{
    @Override //Override not needed for this to work
...
class AllItems<T> //Templated class -> a generic
{
    ...
    public <S> boolean isItem(S v, SearchBehavior<T, S> sb) //Makes sb a polymorphic reference.
    {
        for (T item : _items) // same as a normal for loop. Takes an item in _items and puts it in item. Test it each loop.
        {
            if (sb.search(item, v))
            {
                return true;
    ...
    public <S> int findItem(S v, SearchBehavior<T, S> sb) //<S> says another templated variable.
    {
